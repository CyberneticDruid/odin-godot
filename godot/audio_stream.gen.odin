...
audio_stream__get_tags :: proc "contextless" (
    self: Audio_Stream,
) -> (ret: Dictionary) {
    self := self
    args := []__bindgen_gde.TypePtr {
    }
    __bindgen_gde.object_method_bind_ptrcall(___get_tags_method_ptr, &self, raw_data(args), &ret)
    return
}
...
audio_stream_init :: proc "contextless" () {
    ...
    __name = new_string_name_cstring("_get_tags", true)
    ___get_tags_method_ptr = __bindgen_gde.classdb_get_method_bind(&__class_name, &__name, 3102165223)
...
@(private = "file")
___get_tags_method_ptr: __bindgen_gde.MethodBindPtr
@(private = "file")
