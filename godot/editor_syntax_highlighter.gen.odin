...
editor_syntax_highlighter__create :: proc "contextless" (
    self: Editor_Syntax_Highlighter,
) -> (ret: Editor_Syntax_Highlighter) {
    self := self
    args := []__bindgen_gde.TypePtr {
    }
    __bindgen_gde.object_method_bind_ptrcall(___create_method_ptr, &self, raw_data(args), &ret)
    return
}
...
editor_syntax_highlighter_init :: proc "contextless" () {
    ...
    __name = new_string_name_cstring("_create", true)
    ___create_method_ptr = __bindgen_gde.classdb_get_method_bind(&__class_name, &__name, 3789807118)
}
...
@(private = "file")
___create_method_ptr: __bindgen_gde.MethodBindPtr