...
engine_capture_script_backtraces :: proc "contextless" (
    self: Engine,
    include_variables_: Bool,
) -> (ret: Typed_Array(Script_Backtrace)) {
    self := self
    include_variables_ := include_variables_
    args := []__bindgen_gde.TypePtr {
        &include_variables_,
    }
    __bindgen_gde.object_method_bind_ptrcall(__capture_script_backtraces_method_ptr, &self, raw_data(args), &ret)
    return
}
...
engine_init :: proc "contextless" () {
    ...
    __name = new_string_name_cstring("capture_script_backtraces", true)
    __capture_script_backtraces_method_ptr = __bindgen_gde.classdb_get_method_bind(&__class_name, &__name, 873284517)
...
@(private = "file")
__capture_script_backtraces_method_ptr: __bindgen_gde.MethodBindPtr
@(private = "file")
