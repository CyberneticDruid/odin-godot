...
editor_selection_get_top_selected_nodes :: proc "contextless" (
    self: Editor_Selection,
) -> (ret: Typed_Array(Node)) {
    self := self
    args := []__bindgen_gde.TypePtr {
    }
    __bindgen_gde.object_method_bind_ptrcall(__get_top_selected_nodes_method_ptr, &self, raw_data(args), &ret)
    return
}
...
editor_selection_init :: proc "contextless" () {
    ...
    __name = new_string_name_cstring("get_top_selected_nodes", true)
    __get_top_selected_nodes_method_ptr = __bindgen_gde.classdb_get_method_bind(&__class_name, &__name, 2915620761)
...
@(private = "file")
__get_top_selected_nodes_method_ptr: __bindgen_gde.MethodBindPtr
@(private = "file")
